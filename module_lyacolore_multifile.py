    ...
    def __init__(self, file_paths, parent_sim):
        ...
        try:
            for hdulist in self.hdulists:
                self.RA = np.concatenate( (self.RA, hdulist[1].data['RA']) )
                self.DEC= np.concatenate( (self.DEC,hdulist[1].data['DEC']) )
        except:
            self.RA     = None
            self.DEC    = None 
...
class CoLoReFiles(FilesSkeleton):
    def __init__(self, file_paths, lr_max, parent_sim):
        ...
        self.lr_max             = lr_max
        self.z                  = np.asarray( self.hdulists[0][4].data['Z'])
        self.wavelength         = utils.lya_rest * (1 + self.z)

        self.z_qso              = np.concatenate( [hdulist[1].data['Z_COSMO'] for hdulist in self.hdulists] )
        self.N_qso              = len(self.z_qso)
        self.id                 = list(range(self.N_qso)) # Apparently they do not have an Id so I'll let their name just be a number
        self.mask               = utils.make_IVAR_rows(self.lr_max, self.z_qso, np.log10(utils.lya_rest*(1+self.z)))

        self._delta_skewers     = None
        self._vrad              = None

    @property
    def delta_skewers(self):
        if self._delta_skewers  is  None:
            self._delta_skewers = np.vstack( [hdulist[2].data for hdulist in self.hdulists] )
        return self._delta_skewers
    
    @property
    def vrad(self):
        if self._vrad is None:
            self._vrad          = np.vstack( [hdulist[3].data for hdulist in self.hdulists] )
        return self._vrad
...
class Spectra(FilesSkeleton):
    def __init__(self, arm, spectra_files, truth_files, zbest_files, pixels, lr_max, redshift_to_use, parent_sim):
        ...
        self.z_qso_best = np.concatenate( [hdulist[1].data['Z'] for hdulist in self.zbest.hdulists] )
        self.z_qso_truth= np.concatenate( [hdulist[1].data['Z'] for hdulist in self.truth.hdulists] )
        
        if redshift_to_use == 'best':
            self.z_qso  = self.z_qso_truth
        else:
            self.z_qso  = self.z_qso_best
        
        self.RA         = np.concatenate( [hdulist[1].data['TARGET_RA']  for hdulist in self.hdulists] )
        self.DEC        = np.concatenate( [hdulist[1].data['TARGET_DEC'] for hdulist in self.hdulists] )
        self.id         = np.concatenate( [hdulist[1].data['TARGETID']   for hdulist in self.hdulists] )

        self.wavelength = self.hdulists[0]['{}_WAVELENGTH'.format(self.arm.upper())].data
        self.z          = (self.wavelength - utils.lya_rest)/utils.lya_rest
        self.mask = utils.make_IVAR_rows( self.lr_max, self.z_qso, np.log10(self.wavelength) )
    ...
    def flux(self):
        if self._flux is None:
            self._flux  = np.vstack( [hdulist['{}_FLUX'.format(self.arm.upper())].data for hdulist in self.hdulists ] )
        return self._flux
class Transmission(FilesSkeleton):
    def __init__(self, file_paths, lr_max, parent_sim):
        ...
        self.lr_max         = lr_max
        self.wavelength     = self.hdulists[0][2].data
        self.z              = (self.wavelength - utils.lya_rest)/utils.lya_rest

        self.z_qso          = np.concatenate( [hdulist[1].data['Z']        for hdulist in self.hdulists] )
        self.z_qso_noRSD    = np.concatenate( [hdulist[1].data['Z_noRSD']  for hdulist in self.hdulists] )
        self.id             = np.concatenate( [hdulist[1].data['MOCKID']   for hdulist in self.hdulists] )
        self.N_qso          = len(self.z_qso)
        self.mask           = utils.make_IVAR_rows(lr_max,self.z_qso,np.log10(utils.lya_rest*(1+self.z)))

        self._lya_absorption        = None
        self._lyb_absorption        = None
        self._delta_lya_absorption  = None
        self._delta_lyb_absorption  = None


    @property
    def lya_absorption(self):
        if self._lya_absorption is None:
            self._lya_absorption        = np.vstack(  [hdulist['F_LYA'].data for hdulist in self.hdulists] )
        return self._lya_absorption

    @property
    def lyb_absorption(self):
        if self._lyb_absorption is None:
            self._lyb_absorption        = np.vstack(  [hdulist['F_LYB'].data for hdulist in self.hdulists] )
        return self._lyb_absorption

    @property
    def delta_lya_absorption(self):
        if self._delta_lya_absorption is None:
            self._delta_lya_absorption  = self.lya_absorption/Computations.mean_per_pixel(self.lya_absorption,mask=False)
        return self._delta_lya_absorption
    
    @property
    def delta_lyb_absorption(self):
        if self._delta_lyb_absorption is None:
            self._delta_lyb_absorption  = self.lyb_absorption/Computations.mean_per_pixel(self.lyb_absorption,mask=False)
        return self._delta_lyb_absorption
...
class GaussianCoLoRe(FilesSkeleton):
    def __init__(self,file_paths, lr_max, parent_sim):
        ...
        self.lr_max             = lr_max
        self.z                  = self.hdulists[0][4].data['Z']
        self.wavelength         = utils.lya_rest * (1+self.z)
...
class PiccaStyleFiles(FilesSkeleton):
    def __init__(self,file_paths,lr_max, name, parent_sim):
        ...
        self.__name__       = name 
        self.lr_max         = lr_max
        self.wavelength     = 10**self.hdulists[0][2].data
        self.z              = (self.wavelength - utils.lya_rest)/utils.lya_rest

        self.z_qso          = np.concatenate( [ hdulist[3].data['Z']        for hdulist in self.hdulists] )
        self.N_qso          = len(self.z_qso)
        self.id             = np.concatenate( [ hdulist[3].data['THING_ID'] for hdulist in self.hdulists] )
        self.mask           = utils.make_IVAR_rows(lr_max,self.z_qso,np.log10(utils.lya_rest*(1+self.z)))
    ...
    def values(self):
        if self._values is None:
            self._values    = np.vstack( [hdulist[0].data   for hdulist in self.hdulists] )
            self._values    = self._values.transpose()
        return self._values
